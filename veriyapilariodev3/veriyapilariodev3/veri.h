...
struct IkiliSiralamaAgaci {	//Ikili siralama agaci veri yapisi
    ...
    void agacKapat(Dugum*);		//Agacin dugumleri icin alinan bellek gozlerini program sonlanmadan hemen once iade eder
    void agacOlustur();
};
...
void IkiliSiralamaAgaci::agacKur(int* deger) {
    for (int i = 0; i < 68; i++)
    {
        bool eklendi = false;
        Dugum* tara;
...
inline void IkiliSiralamaAgaci::agacKapat(Dugum*m)
{
    if (m) {
        agacKapat(m->sol);
        agacKapat(m->sag);
...
void IkiliSiralamaAgaci::agacOlustur()
{
    kok = NULL;
}

bool DFS(Dugum *agac, int kontrolsayi) {
    stack<Dugum*>yigin;
    yigin.push(agac);

    while (!yigin.empty()) {
        Dugum* yeni = yigin.top();
        if (kontrolsayi == yeni->veri) {
            return true;
        }
        yigin.pop();
        if (yeni->sag) {
...
bool BFS(Dugum*agac, int kontrolsayi) {
    queue<Dugum*>kuyruk;
    kuyruk.push(agac);

    while (!kuyruk.empty()) {
        Dugum* yeni = kuyruk.front();
        if (kontrolsayi == yeni->veri) {
            return true;
        }
        kuyruk.pop();
        if (yeni->sol) {
            kuyruk.push(yeni->sol);
        }
        if (yeni->sag) {
            kuyruk.push(yeni->sag);
        }
    }
        return false;
}